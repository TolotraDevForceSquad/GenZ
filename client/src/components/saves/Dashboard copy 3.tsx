...
export default function Dashboard() {
  ...
  const updateAlertMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      // ✅ CORRECTION: Suppression de l'alert pour éviter l'affichage d'erreur malgré succès ultérieur
    }
  });

  // ✅ CORRECTION: Mutation pour validation (Confirmer/Rejeter) - Méthode POST + isValid (sans comment)
  const validateAlertMutation = useMutation({
    mutationFn: ({ alertId, isValid, userId }: { alertId: string; isValid: boolean; userId: string }) => {
      console.log(`🔄 Validation alerte ${alertId}: isValid=${isValid}, userId=${userId}`);
  ...
  const handleValidation = (alertId: string, validation: 'confirm' | 'reject' | 'resolved', comment?: string) => {
    ...
    if (validation === 'resolved') {
      // ✅ CORRECTION: Ajout de authorId pour matcher le backend
      updateAlertMutation.mutate({ alertId, status: 'resolved', authorId: currentUserId });
    } else {
      // ✅ CORRECTION: isConfirmed -> isValid (true pour confirm, false pour reject)
  ...
  const handleRejectWithForm = (alertId: string, comment: string) => {
    // ✅ CORRECTION: Comment ignoré (non supporté backend), mais appel validation avec isValid=false
