Bonjour, vous etes un programmer experimente en frontend et backend. Nous voulons recuperer les coordonnees GPS des localisations. Donc par rapport au localisation, c'est tous les quartiers d'antananarivo sans exeption. C est au remplissage de formulaire d inscription que nous avoins besoins des adresses et aussi sur la partie alerte.  et on recupere la geolocalisation apres pour afficher sur map, donne nous le nom fichier a modifer etape par etape avec code avant/ apres, si c est a ajouter, mettre la partie d en haut du code pour que nous puissons indentifier l emplacement du code dans e fichiers. analyse bien et donnne une reponse efficace
```tsx
...
import { searchLocations } from "@/lib/locationsData"; // ✅ AJOUT: Import de la fonction de recherche
